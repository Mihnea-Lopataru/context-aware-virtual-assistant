...
- An exported Windows build in `Deployment/` for running the demo without the
  Unity Editor.
...
|-- Unity Client/             # Unity project source
|-- .gitignore
|-- LICENSE
`-- README.md
```
...
- Unity Editor compatible with the project version in
  `Unity Client/ProjectSettings/ProjectVersion.txt`, if you want to run from
  source or rebuild the exported player
...
### Option 1: Run the Exported Demo Build

The `Deployment/` folder contains the exported Windows player:

```text
Deployment/Unity Client.exe
```

Use this executable for the demo when you do not want to open the Unity Editor.
Start the backend stack first, then run `Deployment/Unity Client.exe`.

The exported build connects to the same local service URLs as the Unity project:
...
Keep the deployment folder together. `Unity Client.exe`, `UnityPlayer.dll`,
`UnityCrashHandler64.exe`, `Unity Client_Data/`, `MonoBleedingEdge/`, and any
other files generated by Unity are part of the same build. Moving only the
`.exe` elsewhere can prevent the application from starting correctly.
